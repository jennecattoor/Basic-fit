...
function Favourites() {
    ...
    return (
        <>
            {(isLoading || recipeLoading) && <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center" minHeight="100vh"><CircularProgress /></Box>}
            ...
            <Box sx={{ color: '#fff', background: 'linear-gradient(0.25turn, #FF9D26, #FE7000);' }}>
                {workouts && workouts.data.filter(workout => workout.attributes.favouriteProfiles.data.find(item => item.id === profileId)).length !== 0 && <Typography variant="h2">Favourite workouts</Typography>}
                {workouts && workouts.data.filter(workout => workout.attributes.favouriteProfiles.data.find(item => item.id === profileId)).length === 0 && <Box
                    display="flex"
            ...
            {recipes && recipes.data.filter(recipe => recipe.attributes.favouriteProfiles.data.find(item => item.id === profileId)).length !== 0 && <Typography variant="h2">Favourite recipes</Typography>}
            <Grid container>
